...
function VoteSeatSharePlotPlanSpecific({title, x_label, y_lable, fips, electionType, characteristic}){
    ...
    useEffect(() => {
        const fetchData = async () => {
            try{
                const response = await axios.get(`http://localhost:8080/NV/SMD/${characteristic}/SEATVOTE`);
            ...
            const totalDemVoteShare =
            simulation.reduce((sum, d) => sum + d.demVoteShare * d.totalVotes, 0) / totalVotes;
            const totalDemSeatShare =
            simulation.filter((d) => d.demVoteShare > 0.5).length / simulation.length;

            dem_df.push({ x: totalDemVoteShare, y: totalDemSeatShare });
            rep_df.push({ x: 1 - totalDemVoteShare, y: 1 - totalDemSeatShare });
            console.log(totalDemVoteShare)
            console.log(totalDemSeatShare)
            // Simulation logic: upward increments
            for (let i = 0; i < 50; i++) {
            let increments = [];
            for (let j = 0; j < 5; j++) {
                increments.push(0.01 + (Math.floor(seed() * 10) - 5) / 100);
            }
            const avgIncrement = increments.reduce((a, b) => a + b, 0) / 5;
    ...
    return (
        <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
          <h2 style={{ marginBottom: "10px" }}>
            {title || `SMD ${characteristic}: Vote-Seat Share`}</h2>
