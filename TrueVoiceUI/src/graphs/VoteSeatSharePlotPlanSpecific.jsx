    ...
    const fetchData = async () => {
      try {
        ...
        for (let i = 0; i < 50; i++) {
          ...
          for (let j = 0; j < 100; j++) {
            const Increment = 0.01 * i + (Math.floor(seed() * 10) - 5) / 100;
          ...
          const aggregatedDemVoteShare = 0.01 * i + totalDemVoteShare;
        ...
        const clampedDemData = dem_df
          .map((d) => ({ x: clamp(d.x), y: d.y }))
          .sort((a, b) => a.x - b.x);
        
        const clampedRepData = rep_df
          .map((d) => ({ x: clamp(d.x), y: d.y }))
          .sort((a, b) => a.x - b.x);
        
        setDemData(clampedDemData);
        setRepData(clampedRepData);
    ...
    <div style={{ display: "flex", flexDirection: "column" }}>
      ...
      <div style={{ position: "relative" }}>
        ...
        <LineChart width={700} height={380} data={demData}>
          ...
          />
          <Legend />
          <Line type="monotone" dataKey="y" name={characteristic === "DEMFAVORED" ? "Democratic" : "Republican"} data={repData} stroke={characteristic === "DEMFAVORED" ? "blue" : "red"} strokeWidth={3} dot={false} isAnimationActive={false} />
          <Line type="monotone" dataKey="y" name={characteristic === "DEMFAVORED" ? "Republican" : "Democratic"} data={demData} stroke={characteristic === "DEMFAVORED" ? "red" : "blue"} strokeWidth={3} dot={false} isAnimationActive={false} />
